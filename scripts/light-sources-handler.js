const SNOW_CUSTOM = true // Für Snow immer true

const NO_LIGHT_SOURCES = -1
const NO_LIGHT_SOURCES_AVAILABLE_OR_CLOSE = -2
const NO_FUEL = -3
...
const HOODED_LANTERN_CLOSED_INFOS = {
...
const HOODED_LANTERN_OPEN_INFOS = {
...
function dropLightItem(token, lightInfos) {
...
function lightSourceIsClose(token, lightSource, distance) {
...
function pickUpLightItem(token, lightInfos) {
...
function createLightSourceButtonObjects(token, lightInfos) {
...
function handleLightEffectAndChangeLight(token, lightInfos) {
...
function extinguishOrDropLightItemDialog(token, lightInfos) {
...
function handleLightItem(token, buttons, lightInfos) {
...
function createButton(label, callbackFunction, active = true) {
    return {
        label: label,
...
function handleLightSpell(token, spellInfos, effect) {
    if (effect) {
...
export function startInchryptianScript(token) {
    ...
    for (let infos of LIGHT_INFO_ORDER.filter(info => info != LIGHT_SPELL_INFOS)) { mainMenuButtons = addItemButtonsToMenu(mainMenuButtons, token, infos) }
